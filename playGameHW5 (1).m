%  Dameli Utembayeva and Olivia Gillman 
...
function randomSearch ()
    while(~allShipsAreSunk())
        %stop the loop when all ships are sunk
        [x,y] = selectNewRandomXYLocation();
        %choose a random point, hit that point 
        [~,~,~] = QueryOpponentBoard (x,y);
        %we put in "~" to get rid of the unecessary outputs 
        %query that point so you don't hit it again
    end
end
...
function neighborhoodSearch (gridSize)
    %first we pass our arrays that store accidentally hit ships 
    global otherShipsX;
    global otherShipsY;
    global index;
    %index is a counter to populate our arrays later
    index = 0;
    
    %stop when al ships are found
    while(~allShipsAreSunk())
        %if the stored accidentally hit neighbors vector is empty
        %then just pick a random point and go thru its neighbors
        if(isempty(otherShipsX))
            [x,y] = selectNewRandomXYLocation();
            [isHit,~,color] = QueryOpponentBoard (x,y); 
            %query the location so we never hit the same location
            %~ in the middle because we never use that output 
            if(isHit)
                %if the point is a hit, only then go thru its neighbors
                findNeighbors(x,y,color);      
            end
        else
            for i=1:index
                %index is the count of our accidentallyhit ships, so it
                %indicated the number of ships we accidentally hit on our
                %way
                % therefore go through the arrays that have accidentally
                % hit neghbors stored and attck those neighbours 
                findNeighbors(otherShipsX(i),otherShipsY(i),...
                    colorOfShipAt(otherShipsX(i),otherShipsY(i)));
                %this finds the neighbors of the stored values 
            end
            index = 0;
            otherShipsX = [];
            otherShipsY = [];
            %at the end, empty the arrays so the loop doesnt infinitely go through
            %those same arrays, because our condition is based on the
            %emptiness of these arrays 
        end
    end
end

function findNeighbors(x,y,color)
%a function that has recursive functions(which all have the same logic)
%and finds all the neighbors from each direction
% we are passing the color because we need to use it to compare if its
% still the same ship we are working with
    moveLeft(x,y,color);
    moveRight(x,y,color);
    moveUp(x,y,color);
    moveDown(x,y,color);
    %before we get into the recursive functions, which is where we store
    %accidentally hit ships, 
    %an overall note for this algorithm is that it if
    %finds an acciedental ship, the algorithm doesnt immediately go to that
    %new ship, it finishes up the current ship and then immediately moves to the new
    %ship
end

function moveLeft(x,y,color)
%first pass global variables 
    global otherShipsX;
    global otherShipsY;
    global index;
    if(isPositionValid(x-1,y)&& ~isPositionQueried(x-1,y))
        % work with the position only if its a valid position which entails that
        %1)its in the grid
        %2) it hasn't been hit 
        [isHit,~,colorNew] = QueryOpponentBoard (x-1,y);
        %query it so we dont go here again
        if(isHit && colorNew~=color)
            %if while finding neighbours we accidentally hit another ship
            %then udate the count and put respective x and y elements in
            %the arrays
            index = index+1; 
            otherShipsX(index) = x-1;
            otherShipsY(index) = y;
        elseif(isHit)
            %if the color doesnt change but it is still a hit, keep going
            %and finding left neighbours
            moveLeft(x-1,y,color);
            %recursion^^^
        end                
    end
end

function moveRight(x,y,color)
%pass global variables 
    ...
    if(isPositionValid(x+1,y)&& ~isPositionQueried(x+1,y))
        ...
        %2) it hasn't been hit
        [isHit,~,colorNew] = QueryOpponentBoard(x+1,y);
        %store accidentally hit ships here 
        if(isHit && colorNew~=color)
            %compare if the ships are the same color, if not, that means
            %they are different ships 
            index = index+1; 
            otherShipsX(index) = x+1;
            %this is the part where we store
            otherShipsY(index) = y;
        elseif(isHit)
            %if the color doesnt change but it is still a hit, keep going
            %and finding right neighbours
            moveRight(x+1,y,color);
        end
    end
end

function moveUp(x,y,color)
%pass globals
    global otherShipsX;
    global otherShipsY;
    global index;
    if(isPositionValid(x,y+1)&& ~isPositionQueried(x,y+1))
        %work with the position only if its a valid position which entails that
        %1)its in the grid
        %2) it hasn't been hit
        [isHit,~,colorNew] = QueryOpponentBoard (x,y+1);
        %store accidentally hit ships in the process here 
        if(isHit && colorNew~=color)
            %if the passed color doesn't correspond to the new color then
            %it's a different ship
            index = index+1; 
            %update counter
            otherShipsX(index) = x;
            otherShipsY(index) = y+1;
            %update both arrays, aka store the accidentally hit values 
        elseif(isHit)
            %if it's just a hit, then just keep going thru neihghbours
            moveUp(x,y+1,color);
        end
    end
end

function moveDown(x,y,color)
%pass globals
    global otherShipsX;
    global otherShipsY;
    global index;
    if(isPositionValid(x,y-1)&& ~isPositionQueried(x,y-1))
        ...
        elseif(isHit)
            %if the color is the same, the ship is still the same ship
            moveDown(x,y-1,color);
        end
    end
end
